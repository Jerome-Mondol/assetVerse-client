...
const UpgradePackage = () => {
    ...
    const [employeeCount] = useState(4)
    const [employeeLimit] = useState(5)
    ...
    const usagePercentage = (employeeCount / employeeLimit) * 100
        ...
        <div className="max-w-6xl mx-auto p-6 text-gray-700">
            ...
            <div className="bg-white rounded-xl shadow p-6 mb-8 border border-blue-200">
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h2 className="text-xl font-bold text-gray-900">Current Plan: <span className="text-blue-600">{currentPlan}</span></h2>
                ...
                <div className="mb-4">
                    <div className="flex justify-between text-sm mb-1">
                        ...
                        <span>{employeeCount} / {employeeLimit} employees</span>
                    </div>
                    ...
                    {employeeCount >= employeeLimit - 1 && (
                        <p className="text-red-500 text-sm mt-2">⚠️ You're nearing your employee limit</p>
                    )}
                </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-12">
                {packages.map((plan) => (
                    <div key={plan._id} className={`bg-white rounded-xl shadow p-6 border-2 ${plan.name === 'Standard' ? 'border-green-500' : 'border-gray-200'} ${plan.name === currentPlan ? 'opacity-80' : ''}`}>
                        {plan.name === 'Standard' && (
                        ...
                        <div className="mb-4">
                            <div className="text-3xl font-bold mb-1">${plan.price}<span className="text-gray-500 text-lg">/month</span></div>
                        ...
                        <button 
                            className={`w-full py-3 rounded-lg font-semibold ${plan.name === currentPlan ? 'bg-gray-300 text-gray-600' : plan.name === 'Standard' ? 'bg-green-500 hover:bg-green-600' : 'bg-blue-500 hover:bg-blue-600'} text-white transition-colors`}
                            disabled={plan.name === currentPlan}
                        >
                            {plan.name === currentPlan ? 'Current Plan' : plan.name === 'Basic' ? 'Downgrade' : 'Upgrade Now'}
